...
import 'package:innolab/src/features/core/client/request/requestController/request_controller.dart';
...
class _RequestOrder {
  ...
  factory _RequestOrder.fromDocument(
    QueryDocumentSnapshot<Map<String, dynamic>> doc,
  ) {
    final data = doc.data();

    final statusRaw =
        data['status']?.toString() ?? data['requestStatus']?.toString();
    ...
    final timeline = [
      _StatusStep(
        ...
        time: submittedAt.toLocal().toString(),
        done: true,
      ),
      _StatusStep(
        label: status.label,
    ...
    final printer = data['printer'] as Map<String, dynamic>? ?? {};
    final line = data['line'] as Map<String, dynamic>? ?? {};
    final pricing = data['pricing'] as Map<String, dynamic>? ?? {};

    return _RequestOrder(
      id: doc.id,
      fileName: model['fileName']?.toString() ?? 'Unknown',
      material: material['name']?.toString() ?? 'Unknown',
      printer: printer['name']?.toString() ?? 'Unassigned',
      quantity: _asInt(line['quantity'] ?? 1),
      grandTotal: _asDouble(pricing['grandTotal'] ?? 0),
      status: status,
...
class _UserRequestScreenState extends State<UserRequestScreen>
  ...
  Stream<List<_RequestOrder>>? _requestStream;
  ...
  void initState() {
    ...
    if (user != null) {
      _requestStream = UserRequestController.instance
          .watchUserQuoteRequests(user.uid)
          .map((snapshot) {
            final orders = snapshot.docs
                .map((doc) => _RequestOrder.fromDocument(doc))
                .toList();
  ...
  List<_RequestOrder> _filteredOrders(
    RequestStatus? status,
    List<_RequestOrder> orders,
  ) => status == null
      ? orders
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<_RequestOrder>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return Center(
            child: Text('Error loading your requests: ${snapshot.error}'),
          );
        }
              ...
              child: Container(
                ...
                child: TabBar(
                  ...
                  tabs: List.generate(_tabLabels.length, (i) {
                    final count = _filteredOrders(_tabs[i], orders).length;
                    return Tab(
                      child: Padding(
            ...
            Expanded(
              child: TabBarView(
                ...
                children: _tabs.map((status) {
                  final filtered = _filteredOrders(status, orders);
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (_, i) => _OrderCard(
                      ...
                      onToggle: () => setState(() {
                        _expandedOrderId = _expandedOrderId == filtered[i].id
                            ? null
                            : filtered[i].id;
                      }),
                    ),
      ...
      return SizedBox(
        ...
        child: OutlinedButton.icon(
          onPressed: () async {
            ...
            if (confirmed == true) {
              try {
                await UserRequestController.instance.deleteQuoteRequest(
                  order.id,
                );
                if (mounted) {
